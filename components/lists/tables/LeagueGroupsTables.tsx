...
import { LeagueGroupWithTeams } from "@/types";
import { sortGroupTeams } from "@/lib/sortGroupTeams";

import PageHeader from "@/components/PageHeader";
import NoDataFoundComponent from "@/components/NoDataFoundComponent";
import LeagueGroupsFilterDialog from "@/components/lists/tables/LeagueGroupsFilterDialog";
...
export default function LeagueGroupsTables({
  season,
  groupsWithTeams,
}: {
  season: LeagueSeasonProps;
  groupsWithTeams: LeagueGroupWithTeams[];
}) {
  return (
    <>
      <PageHeader label={`${season?.league.name} ${season?.year} Groups`} />
      <div className="flex justify-end pb-2">
        <LeagueGroupsFilterDialog groups={season.groups} />
      </div>
